...
import { useGetComboToursQuery } from "@/redux/api/features/tour/tourApi";
...
export default function ComboSection({ className = "" }: ComboSectionProps) {
  const router = useRouter();

  const { data, isLoading } = useGetComboToursQuery({ limit: 12 });
  ...
  const infiniteTours = useMemo(() => {
    return tours.length > slidesToShow ? [...tours, ...tours, ...tours] : tours;
  }, [tours, slidesToShow]);
    ...
    return (
      <section className={`bg-white py-12 md:py-20 ${className}`}>
        <div className="container mx-auto px-4 text-center">
          ...
          <p className="text-gray-600">Loading combo tours...</p>
        </div>
      </section>
    );
  }
  ...
  return (
    <section className={`bg-white py-12 md:py-20 ${className}`}>
      <div className="container mx-auto px-4">
        <HeadingSection
          badgeText="combo tours"
          badgeIcon="🎉"
          title="Combo Tours"
          subtitle="Explore bundled travel experiences that combine the best destinations in one package"
        />
        ...
        >
          ...
          <Button
            ...
            onClick={() => router.push("/package-details?category=combo")}
          >
            <span className="hidden sm:inline">View All Combo Tours</span>
